...
const passParentPath = (children, path) =>
	React.Children.map(children, child =>
		React.cloneElement(child, {parentPath: path, ...child.props})
	);
...
const Docs = ({component, location, children, ...props}) => {
				...
				return (
					<div
						className={classNames(
							'ptr-docs-nav-directory',
							'level' + level,
							specificClass,
							className,
							{active: current, activeDescendant: includesCurrent}
						)}
					>
						<span className="ptr-docs-nav-link">
							<NavLink to={path}>{label}</NavLink>
						</span>
						<div style={colour ? {borderColor: colour} : null}>
							{React.Children.map(
								children,
								processNode.bind(this, level + 1, currentPath, path)
							)}
						</div>
					</div>
				);
	...
	const tree = React.Children.map(
		children,
		processNode.bind(this, 1, location.pathname, '/')
	);
		...
		<div className="ptr-docs ptr-light">
			<div className="ptr-docs-nav">
				<div className="ptr-docs-nav-header">
					<span className="ptr-docs-nav-header-title">
						<NavLink to="/">Panther docs</NavLink>
					</span>
				</div>
				<div className="ptr-docs-nav-tree">{tree}</div>
			</div>
