...
class TimelineDoc extends React.PureComponent {
	getHoverContent(x, time) {
		return <div>time: {` ${time.format('YYYY MM D H:mm:ss')}`}</div>;
	}
	...
	onOverlayClick = overlays => {
		console.log('On overlay click', overlays);
	};
	...
	render() {
		...
		return (
			<Page title="Timeline">
				<p>
					Timeline is a container for svg content. User can drill by zoom into
					small scale or gets an overview of the whole periodLimit. Timeline is
					prepared for vertical or horizontal view.
				</p>

				<p>
					The main idea is, that container has defined periodLimit with start
					and end. Container calculate dayWidth in px as container width
					(height) / visible period. Passed children content gets dayWidth and
					visible period in props. DayWidth could be modified by zoom/pinch or
					programmatically.
				</p>
				...
				<ComponentPropsTable
					content={[
						...
						},
						{
							...
							required: false,
							description:
								'Zoom can be modified by dayWidth. DayWidth is size of one day in px. If component gets onMount props periodLimit and dayWidth, new dayWidth is calculated from periodLimit.',
						},
						{
							name: 'contentHeight',
							type: 'number',
				...
				<p>
					As said, timeline is a container with custom children. Children
					variously display time, or data, layers even graphs. Custom child is
					also mouse indicator, periods limitations or center of timeline.
				</p>
				<p>
					CenterPicker and Mouse are core component, but it is possible to pass
					custom children.
				</p>
				<p>
					Timeline has onChange and onClick listeners that write events into
					console. Open developer console to see content.
				</p>
...
<Timeline 
	...
	>
		...
		<Mouse mouseBufferWidth={20} key="mouse"/>
				...
				<h3>Levels</h3>
				<p>
					Timeline can render children in pyramid depends on dayWidth. Every
					level has max dayWidth value until is visible. While rendering years
					it does not care about rendering minutes or seconds, it has positive
					performance aspects. In one moment is visible only one layer of the
					period.
				</p>
				<SyntaxHighlighter language="jsx">
					{`
...
`}
				...
				<h3>Vertical</h3>
				<p>
					Vertical view is definded by prop{' '}
					<InlineCodeHighlighter>vertical: true</InlineCodeHighlighter>.
					Timeline children get vertical information in props and they should
					adapt to vertical.
				</p>
				<SyntaxHighlighter language="jsx">
					{`
...
`}
				...
				<h3>Tooltip</h3>
				<p>
					It's possible to show tooltip for horizontal or vertical view of
					timeline. Tooltip content is handle by{' '}
					<InlineCodeHighlighter>TimeLineHover</InlineCodeHighlighter>. Position
					of tooltip is handle by{' '}
					<Link to="/docs/components/iHaveNoIdea/hoverHandler">
						<InlineCodeHighlighter>HoverHandler</InlineCodeHighlighter>
					</Link>
					.
				</p>
				<SyntaxHighlighter language="jsx">
...
`}
				...
				<h4>Tooltip in vertical view</h4>
				<p>
					Vertical view with{' '}
					<InlineCodeHighlighter>
						const referencePoint = 'center';
					</InlineCodeHighlighter>{' '}
					and position of tooltip in{' '}
