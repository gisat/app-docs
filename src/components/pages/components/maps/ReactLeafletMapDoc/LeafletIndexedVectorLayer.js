		...
		if (layer === 'cz_large_data_layer') {
			...
			this.setState({
				...
				loading: features.length
					? [...this.state.loading, 'Add data to map']
	...
	render() {
		return (
			<Page title="Indexed Vector layer">
				<p>
					Indexed Vector layer wraps both{' '}
					...
					<Link to="/architecture/systemDataTypes/layers#vector">
						<InlineCodeHighlighter>boxRangeRange</InlineCodeHighlighter>
					</Link>{' '}
					property to limit range where layer is rendered - useful for layers
				...
				<h3>Point</h3>
				<div>
					First, load desired amount of point features (and then optionally
					remove them again):
				</div>
				<Buttons>
					<Button onClick={this.addData.bind(this, 1)}>
						Add 20 000 points
					</Button>
					<Button onClick={this.addData.bind(this, 10)}>
						Add 200 000 points
					</Button>
				...
				<div className="ptr-docs-process-info-box">
					{this.state.loading &&
						this.state.loading.map((step, i) => <div key={i}>{step}</div>)}
				</div>

				<div>
					Next, you can optionaly switch the maximum range to draw the features:{' '}
				</div>
				<ButtonGroup>
					<ButtonSwitchOption
						primary={this.state.maxBoxRange === 20000}
						onClick={this.onMaxRangeChange.bind(this, 20000)}
					>
						20 km
					</ButtonSwitchOption>
					<ButtonSwitchOption
						primary={this.state.maxBoxRange === 50000}
						onClick={this.onMaxRangeChange.bind(this, 50000)}
					>
						50 km
					</ButtonSwitchOption>
					<ButtonSwitchOption
						primary={this.state.maxBoxRange === 100000}
						onClick={this.onMaxRangeChange.bind(this, 100000)}
					>
						100 km
					</ButtonSwitchOption>
				</ButtonGroup>
				...
				<div style={{marginTop: '1rem'}}>
					Zoom in/out to see current range change:{' '}
					<b>{Math.round(this.state.currentPointsBoxRange / 1000)}</b> km
				</div>
				...
				<div style={{height: 500, marginBottom: 10}}>
					<HoverHandler
						popupContentComponent={props => (
							<>
								Value for color: <b>{props.data['a1']}</b>
								<br />
								Value for size: <b>{props.data['a3']}</b>
								<br />
				...
				<h3>Polygons</h3>
				<div>
					First, load point features (and then optionally remove them again):
				</div>
				<Buttons>
					<Button onClick={this.addPolygonData.bind(this)}>
						Add 1500 polygons
					</Button>
					<Button onClick={this.removePolygonData.bind(this)}>
						Remove polygon data
					</Button>
				</Buttons>
				<div style={{marginTop: '1rem'}}>
					Min box range to draw features:{' '}
					{default_polygons_boxRangeRange[0] / 1000} km
				</div>
				<div>
					Max box range to draw features:{' '}
					{default_polygons_boxRangeRange[1] / 1000} km
				</div>
				<div>
					Zoom in/out to see current range change:{' '}
					<b>{Math.round(this.state.currentPolygonsBoxRange / 1000)}</b> km
				</div>
