			...
			<Page title="ReactLeafletMap">
				...
				<ComponentPropsTable>
					<Prop name="mapKey" required type="string">
						Key required by leaflet to setup a map
					</Prop>
				...
				<ul className="ptr-docs-basic-list">
					...
					<li>
						<Link to="./reactLeaflet/WMSLayer">WMS layer</Link>
					</li>
					<li>
...
`}
				...
				<p>
					...
					places on the earth. The decision as to which map projection and
					coordinate reference system to use, depends on the regional extent of
					the area you want to work in.
				</p>
				<p>
					Currently, it is possible to use following CRS in ReactLeafletMap:{' '}
				</p>
				<ul className="ptr-docs-basic-list">
				...
				<p>
					GCS defines where the data is located on the earth’s surface. PCRS
					tells the data how to draw on a flat surface, like on a paper map or a
					computer screen. The GCS is round, and so records locations in
					degrees. The PCRS is flat, so it records locations in linear units
					(usually meters).
				</p>
...
>
	<MapControls zoomOnly levelsBased/>
				...
				<p>
					Alternatively, you can set crs for WMS layer only (e.g. in cases,
					where WMS is not provided in Leaflet's default crs - EPSG:3857) from
					the list above.
				</p>
