	...
	render() {
		return (
			<Page title="Leaflet diagram layer">
				<p>
					Use diagram to visualize absolute value for area. See the examples
					bellow how diagrams can be drawn on the top of choropleth map.
				</p>

				<p>
					DiagramLayer component extends{' '}
					<Link to="./vectorLayer">VectorLayer</Link> component and it is always
					used inside ReactLeafletMap component. The data are passed via layers
					prop (see <Link to="/components/maps/map">Map</Link> documentation),
					where each layer is represented by Vector layer data type. For general
				...
				<p>
					In case of Diagram layer, following options are different from Vector
				...
				<div style={{height: 500, marginBottom: 10}}>
					<HoverHandler
						popupContentComponent={props => {
							return (
								<>
									<b>{props.data['GID_1']}</b>:{' '}
									{props.data['e575b4d4-7c7a-4658-bb9a-a9b61fcc2587']
										? props.data[
												'e575b4d4-7c7a-4658-bb9a-a9b61fcc2587'
										  ].toFixed(2)
										: null}
								</>
							);
						...
						popupContentComponent={props => {
							return (
								<>
									...
									Diagram: {props.data['positive_attr'].toFixed(2)}
									<br />
									Choropleth: {props.data['diverging_attr'].toFixed(2)}
									<br />
								</>
							);
						}}
