...
class TileGridDoc extends React.PureComponent {
	...
	render() {
		const mergedView = {
			...view,
			center: {...view.center, ...this.state.map1.center},
			boxRange: this.state.map1.boxRange || view.boxRange,
		};
		...
		const gridLayer = {
			...
			options: {
				features:
					this.state.map1.geojsonTileGrid &&
					this.state.map1.geojsonTileGrid[this.state.map1.level]
						? this.state.map1.geojsonTileGrid[this.state.map1.level].features
						: [],
				style: polygonsStyle,
				selected: {
					testSelection: {
		...
		return (
			<Page title="WorldWindMap">
				<p>
					@gisatcz/ptr-tile-grid is utility for work with wgs tileset.{' '}
					<a
						href={'https://github.com/gisat-panther/ptr-tile-grid'}
						target={'_blank'}
						rel="noopener noreferrer"
					>
						https://github.com/gisat-panther/ptr-tile-grid
					</a>
				</p>
